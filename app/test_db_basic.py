    ...
    def add_recipe(self, recipe_data: dict) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"Failed to add recipe (Status {response.status_code}):")
                try:
    ...
    def list_recipes(self) -> list:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"Failed to fetch recipes (Status {response.status_code}):")
                try:
                    print(f"  {response.json().get('detail', 'Unknown error')}")
                except:
                    print(f"  Response: {response.text[:200]}")
    ...
    def delete_recipe(self, recipe_id: int) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"Failed to delete recipe (Status {response.status_code}):")
                try:
                    print(f"  {response.json().get('detail', 'Unknown error')}")
                except:
                    print(f"  Response: {response.text[:200]}")
